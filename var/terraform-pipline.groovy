...
EOF"""
                  ...
                  def projectName = env.JOB_NAME.split("/")[1]
                  def orgName = projectName.split("-")[0,1].join()
                  def jscoveragePath = 'coverage/lcov.info'
                  def reportPath = 'coverage/clover.xml'
...
EOF"""
                  ...
                  sh "npm run test"
                  sh """
                    if [[ -f "$jscoveragePath" ]]; then
                      echo sonar.javascript.lcov.reportPaths=${jscoveragePath}   >> sonar-project.properties
                    if [[ -f "${reportPath}"]]; then
                      echo sonar.clover.reportPath=${reportPath}   >> sonar-project.properties
                ...
                } else if (fileExists("src/secretexpirychecker_test.py")){
                  def scannerHome = tool 'Sonar Scanner';
                  def projectName = env.JOB_NAME.split("/")[1]
                  def orgName = projectName.split("-")[0,1].join()
                  def coveragePath = 'coverage-reports/*coverage-*.xml'
                  sh """cat << EOF > sonar-project.properties
sonar.projectKey=com.mcd.${orgName}:${projectName}
sonar.projectName=${projectName}
EOF"""            
